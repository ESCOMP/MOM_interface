...
class MARBL_settings_for_MOM(object):
    def __init__(
        ...
        for var in ["base_bio_on", "abio_dic_on"]:
            MARBL_args["exclude_dict"][var] = (
                f"ERROR: {var} can not be set in user_nl_marbl -- "
                + "it is controlled by MARBL_TRACER_OPTS in env_run.xml"
            )
        for var in ["ciso_on", "ladjust_bury_coeff"]:
            MARBL_args["exclude_dict"][
                var
            ] = f"ERROR: running with {var} is not supported in CESM+MOM6 yet"

        # User can put settings_file in SourceMods, otherwise use file provided by MARBL
        MARBL_args["default_settings_file"] = os.path.join(
            caseroot, "SourceMods", "src.mom", settings_file
        )
        if not os.path.isfile(MARBL_args["default_settings_file"]):
            MARBL_args["default_settings_file"] = os.path.join(
                MARBL_dir, "defaults", "json", settings_file
            )
        ...
        if not os.path.isfile(
            os.path.join(settings_class_dir, "MARBL_settings_file_class.py")
        ):
            ...
            logging.info(
                "Importing MARBL_settings_file_class.py from %s" % settings_class_dir
            )
            settings_class_module = settings_class_dir + "/MARBL_settings_file_class.py"
            if os.path.isfile(settings_class_module):
                MARBL_settings_file_class = imp.load_source(
                    "MARBL_settings_file_class", settings_class_module
                )
            else:
        ...
        self._MARBL_settings = MARBL_settings_file_class.MARBL_settings_class(
            **MARBL_args
        )
    ...
    def get_autotroph_names(self, calcifier_only=False):
        ...
        try:
            for n in range(
                1, self._MARBL_settings.settings_dict["autotroph_cnt"]["value"] + 1
            ):
                autotroph_name = self._MARBL_settings.settings_dict[
                    "autotroph_settings(%d)%%sname" % n
                ]["value"].strip('"')
                imp_calcifier = (
                    self._MARBL_settings.settings_dict[
                        "autotroph_settings(%d)%%imp_calcifier" % n
                    ]["value"].strip('"')
                    == ".true."
                )
                exp_calcifier = (
                    self._MARBL_settings.settings_dict[
                        "autotroph_settings(%d)%%exp_calcifier" % n
                    ]["value"].strip('"')
                    == ".true."
                )
    ...
    def get_zooplankton_names(self):
        ...
        try:
            for n in range(
                1, self._MARBL_settings.settings_dict["zooplankton_cnt"]["value"] + 1
            ):
                zooplankton_name = self._MARBL_settings.settings_dict[
                    "zooplankton_settings(%d)%%sname" % n
                ]["value"].strip('"')
    ...
    def get_ladjust_bury_coeff(self):
        ...
        try:
            return (
                self._MARBL_settings.settings_dict["ladjust_bury_coeff"]["value"].strip(
                    '"'
                )
                == ".true."
            )
